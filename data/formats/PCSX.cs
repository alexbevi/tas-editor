...
namespace MovieSplicer.Data.Formats
{
    ...
    {
        private static int[] BYTES_PER_FRAME = { 0, 4, 0, 0, 2, 6, 0, 6 };
        ...
        public PCSX(string PXMFile)
        {
            ...
            Extra = new TASExtra();
            Extra.ROM = ReadChars(ref FileContents, PXMSpecific.CDRomIDOffset + 1, FileContents[PXMSpecific.CDRomIDOffset] * 9);
        ...
        private void getFrameInput(ref byte[] byteArray)
        {
            ...
            for (int i = 0, pos = ControllerDataOffset; i < Header.FrameCount; ++i)
            {
                ...
                Input.FrameData[i].Controller[0] = parseStandardControllerData(byteArray, pos);
                pos += Controller1Size;
                Input.FrameData[i].Controller[1] = parseStandardControllerData(byteArray, pos);
                pos += Controller2Size;
        ...
        public override void Save(string filename, ref TASMovieInput[] input)
        {
            ...
            {
                byte[] parsed          = parseStandardControllerData(input[i].Controller[0]);
                outputFile[position++] = parsed[0];
                outputFile[position++] = parsed[1];
                parsed                 = parseStandardControllerData(input[i].Controller[1]);
                outputFile[position++] = parsed[0];
                outputFile[position++] = parsed[1];
                outputFile[position++] = parseControlData(input[i].Controller[2])[0];
            }
